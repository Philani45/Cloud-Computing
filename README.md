...
## 9) What to do after infrastructure is up

Once `terraform apply` succeeds and your ALB DNS responds, continue in this order:

1. **Deploy the backend app** to both EC2 instances (same build/version on each instance).
   - Keep a stable health endpoint (example: `/health`) for ALB checks.
2. **Update ALB health check path** to match your backend endpoint if you change app routes.
3. **Add your frontend pages/templates** for course listing and registration confirmation.
4. **Prove load distribution** by displaying hostname or instance-id in responses and refreshing multiple times.
5. **Harden and automate**: HTTPS (ACM + 443 listener), CI/CD deploys, CloudWatch alarms/logging.

For this repository specifically, the current bootstrap script installs Apache and serves static HTML; replacing that bootstrap with your Flask app startup is the next practical step for the CSC registration demo.
